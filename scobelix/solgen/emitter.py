...
(scobelix.decompiler.Decompilation.json, i.e. Contract.json()) into Solidity
source text.
...
def _op(exp):
    """Like scobelix.utils.helpers.opcode(), but tolerant of the plain
...
class SolidityEmitter:
    ...
    def _log_stmt(self, stmt, indent):
        # ("log", data_expr, *topics) - see scobelix/vm.py's `elif op[:3]
    ...
    def _expr(self, exp):
        ...
        if op == "mask_shl" and len(exp) == 5:
            ...
            if size == _ADDRESS_MASK_SIZE:
                return f"address(uint160({self._expr_as_uint256(val)}))"
            if off >= 0:
                # unambiguous case (see scobelix.core.masks.mask_to_int and
...
def generate_solidity(decompilation) -> SolgenResult:
    """Entry point: decompilation is a scobelix.decompiler.Decompilation."""
