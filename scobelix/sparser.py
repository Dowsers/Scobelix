...
from scobelix.core.algebra import divisible_bytes, minus_op, safe_le_op, to_bytes
from scobelix.core.masks import mask_to_type
from scobelix.matcher import Any, match
from scobelix.prettify import pprint_trace, pretty_stor
from scobelix.utils.helpers import (
    COLOR_GRAY,
    COLOR_GREEN,
    ENDC,
    find_f,
...
def get_name_full(exp):
    def f(exp):
        if type(exp) != tuple:
            ...
            "stor",
            "storage",
        ):  # exp ~ ('storage', ...): # if we have a storage reference within the storage
            # don't follow this one when looking for loc
            return None

        elif match(exp, ("name", ":name", Any)):
            return exp
...
def find_stores(exp):
    if m := match(exp, ("store", ":size", ":off", ":idx", ":val")):
        res = {("storage", m.size, m.off, m.idx)}

    elif opcode(exp) == "storage":
        res = {exp}
...
def rewrite_functions(functions):
    ...
    def get_type(stordefs):
        ...
        for s in stordefs:
            if (
                m := match(s, ("stor", ":size", ":off", (":op", ":idx", ...)))
            ) and m.op in ("map", "array"):
                sizes.add(m.size)
        ...
        if len(offsets) > 0:
            ...
            for s in stordefs:
                if (
                    m := match(s, ("stor", Any, Any, (":op", ":idx", ...)))
                ) and m.op in ("map", "array"):
                    idx = m.idx
                    if (
                        m := match(idx, ("stor", Any, ":int:siz", ("length", ...)))
                    ) and m.siz < 0:
                        return ("struct", -m.siz + 1)
    ...
    for loc in sort(stordefs.keys()):
        for l in sort(stordefs[loc]):
            if match(l, ("stor", int, int, ("loc", Any))) or match(
                l, ("stor", int, int, ("name", ...))
            ):
                continue
...
def replace_names_in_assoc_bool(names, storages_assoc):
    ...
    for getter, name in names.items():
        if not (m := match(getter, ("bool", ("storage", ":size", ":off", ":int:loc")))):
            continue
        size, off, loc = m.size, m.off, m.loc
...
def replace_names_in_assoc(names, storages_assoc):
    for pattern, name in names.items():
        ...
        if m := match(stor_id, ("stor", ":size", ":off", ("loc", ":num"))):
            size, off, num = m.size, m.off, m.num
            ...
            (m := match(stor_id, ("stor", Any, Any, ("map", Any, ":loc"))))
...
def mask_to_mul(exp):
    if m := match(exp, ("mask_shl", ":int:size", ":int:offset", ":int:shl", ":val")):
...
def _sparser(orig_storages):
    ...
    for s in storages:
        ...
        if m := match(idx, ("add", ":int:num", ...)):
            terms = idx[2:]
            if get_loc(terms) is not None:
                offset += 256 * m.num
                idx = ("add",) + tuple(terms)
    ...
    for s in storages:
        if m := match(
            ...
            s = ("stor", m.size, m.off, ("array", ("range", m.beg, m.end), m.loc))
    ...
    def double_map(exp):
        ...
        if m := match(exp, ("add", ":idx", ("map", ...))):
            terms = exp[2][1:]
            return ("array", m.idx, ("map", *terms))
        if m := match(exp, ("sha3", ":idx", ":loc")):
            return ("map", m.idx, m.loc)

        # this all should be recursive
        if m := match(exp, ("add", ("sha3", ("array", ...)), ":num")):
            terms = exp[1][1][1:]
            return ("array", m.num, ("array",) + tuple(terms))
        if m := match(exp, ("add", ":num", ("sha3", ("array", ...)))):
            terms = exp[2][1][1:]
            return ("array", m.num, ("array",) + tuple(terms))

        if m := match(exp, ("sha3", ("array", ":idx", ":loc"))):
        ...
        if m := match(exp, ("add", ("var", ":x"), ("array", ":idx", ":loc"))):
            return ("array", ("add", m.idx, ("var", m.x)), m.loc)

        if m := match(exp, ("add", ("array", ":idx", ":loc"), ("var", ":x"))):
            return ("array", ("add", m.idx, ("var", m.x)), m.loc)
...
sha3_loc_table = {
    ...
    "0x4ad3b33220dddc71b994a52d72c06b10862965f7d926534c05c00fb7e819e7b7": (
        "map",
        0,
        ("loc", 17),
    ),
    "0x7e7fa33969761a458e04f477e039a608702b4f924981d6653935a8319a08ad7b": (
        "map",
        0,
        ("loc", 18),
    ),
    "0x8fa6efc3be94b5b348b21fea823fe8d100408cee9b7f90524494500445d8ff6c": (
        "map",
        0,
        ("loc", 19),
    ),
    ...
    "0xad67d757c34507f157cacfa2e3153e9f260a2244f30428821be7be64587ac55f": (
        "map",
        1,
        ("loc", 8),
    ),
    ...
    "0x169f97de0d9a84d840042b17d3c6b9638b3d6fd9024c9eb0c7a306a17b49f88f": (
        "map",
        1,
        ("loc", 15),
    ),
    ...
    "0x17bc176d2408558f6e4111feebc3cab4e16b63e967be91cde721f4c8a488b552": (
        "map",
        1,
        ("loc", 17),
    ),
    "0x71a67924699a20698523213e55fe499d539379d7769cd5567e2c45d583f815a3": (
        "map",
        1,
        ("loc", 18),
    ),
    "0x4155c2f711f2cdd34f8262ab8fb9b7020a700fe7b6948222152f7670d1fdf34d": (
        "map",
        1,
        ("loc", 19),
    ),
